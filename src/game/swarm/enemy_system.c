...
static EnemySpawnPreset g_SpawnPresets[] = {
    {SPAWN_SINGLE, 60.0f, {1, 1}, {500.0f, 700.0f}, ENEMY_BASIC, 0.0f},
    {SPAWN_LINE, 15.0f, {8, 12}, {700.0f, 1000.0f}, ENEMY_BASIC, 0.0f},
    {SPAWN_CLUSTER, 20.0f, {8, 20}, {600.0f, 800.0f}, ENEMY_BASIC, 0.0f},
    ...
    {SPAWN_LINE, 16.0f, {40, 60}, {900.0f, 1000.0f}, ENEMY_FAST, 45.0f},
    {SPAWN_CLUSTER, 20.0f, {60, 80}, {700.0f, 900.0f}, ENEMY_TANK, 55.0f},
    {SPAWN_SURROUND, 18.0f, {100, 150}, {1500.0f, 1500.0f}, ENEMY_BASIC, 65.0f},
    {SPAWN_CLUSTER, 30.0f, {60, 90}, {800.0f, 1000.0f}, ENEMY_FAST, 75.0f},
    {SPAWN_LINE, 20.0f, {100, 120}, {1200.0f, 1300.0f}, ENEMY_TANK, 85.0f},
    {SPAWN_SURROUND, 40.0f, {200, 250}, {1600.0f, 1800.0f}, ENEMY_FAST, 95.0f},
    {SPAWN_SURROUND,
     50.0f,
     {300, 500},
     {2000.0f, 2200.0f},
...
static Vector2 GetRandomPointAtDistance(Vector2 origin, float minDist,
                                        float maxDist) {
  float angle = (float)GetRandomValue(0, 360) * DEG2RAD;
  float dist = (float)GetRandomValue((int)minDist, (int)maxDist);
  return (Vector2){origin.x + cosf(angle) * dist,
...
void EnemySystem_TriggerPreset(Vector2 playerPos, EnemySpawnPreset preset,
  ...
  if (preset.enemyType == ENEMY_MINI_BOSS) {
    enemyColor = GREEN;
    baseSpeed = 70.0f;
    health = 3000;
    damage = 40;
  ...
  case SPAWN_SINGLE: {
    for (int i = 0; i < count; i++) {
      Vector2 pos = GetRandomPointAtDistance(playerPos, preset.distRange.min,
                                             preset.distRange.max);
  ...
  case SPAWN_LINE: {
    Vector2 spawnCenter = GetRandomPointAtDistance(
        playerPos, preset.distRange.min, preset.distRange.max);
    Vector2 toPlayer =
        Vector2Normalize(Vector2Subtract(playerPos, spawnCenter));
  ...
  case SPAWN_CLUSTER: {
    Vector2 spawnCenter = GetRandomPointAtDistance(
        playerPos, preset.distRange.min, preset.distRange.max);
    for (int i = 0; i < count; i++) {
      Vector2 offset = {(float)GetRandomValue(-60, 60),
                        (float)GetRandomValue(-60, 60)};
      float speed = baseSpeed + (float)GetRandomValue(-20, 20);
      ECS_SpawnEnemy(Vector2Add(spawnCenter, offset), enemyColor, size, health,
                     speed, damage, preset.enemyType);
    }
  } break;

  case SPAWN_SURROUND: {
    float dist =
        preset.distRange.min; // Surround uses the min as the radius usually
    for (int i = 0; i < count; i++) {
      float angle = (i / (float)count) * PI * 2.0f;
      Vector2 pos = {playerPos.x + cosf(angle) * dist,
...
void EnemySystem_Update(float deltaTime, Vector2 playerPos,
  ...
  // Swarm Event logic
  float swarmCycle = fmodf(playerState->gameTime, 120.0f);
  bool bIsSwarming = (playerState->gameTime >= 120.0f && swarmCycle < 20.0f);
  if (bIsSwarming) {
    spawnSpeedMultiplier = 5.0f;
  } else if (playerState->gameTime > 30.0f && aliveCount < 100) {
    spawnSpeedMultiplier = 1.5f;
  }

  // 3. Mini Boss Spawning (Every 3 minutes)
  static float g_LastBossSpawn = 0.0f;
  if (playerState->gameTime >= 180.0f &&
      (playerState->gameTime - g_LastBossSpawn) >= 180.0f) {
    g_LastBossSpawn = playerState->gameTime;
  ...
  if (g_SpawnTimer >= g_SpawnRate) {
    ...
    float currentDifficulty = (playerState->gameTime / 10.0f) * 2.0f;

    // Filter eligible presets
    int eligibleIndices[PRESET_COUNT];
    int eligibleCount = 0;
    float totalWeight = 0;

    for (int i = 0; i < PRESET_COUNT; i++) {
      if (g_SpawnPresets[i].difficulty <= currentDifficulty) {
        eligibleIndices[eligibleCount++] = i;
        totalWeight += g_SpawnPresets[i].weight;
      }
    }
    ...
    if (eligibleCount > 0) {
      ...
      for (int i = 0; i < eligibleCount; i++) {
        ...
        currentWeight += g_SpawnPresets[idx].weight;
        if (roll <= currentWeight) {
          EnemySystem_TriggerPreset(playerPos, g_SpawnPresets[idx], minutes);
          break;
        }
      }
    }
      ...
      if (enemy_attackTimers[i] <= 0.0f) {
        ...
        for (int j = 0; j < MAX_ENEMIES; j++) {
          if (i != j && enemy_bIsActive[j]) {
            Vector2 toOther =
                Vector2Subtract(enemy_positions[i], enemy_positions[j]);
            ...
            if (distSq < repulseRadius * repulseRadius && distSq > 0.01f) {
              ...
              separation = Vector2Add(
                  separation, Vector2Scale(repulse, moveSpeed * force * 2.5f));
            }
          }
        }
